...
const TranscriptPanel: React.FC<TranscriptPanelProps> = ({
  ...
  return <div className="flex flex-col h-full">
      <div className="p-4 border-b bg-slate-50">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant={transcript.callActive ? "destructive" : "default"} onClick={transcript.handleCall}>
      ...
      <ScrollArea className="flex-grow p-4">
        {transcript.verificationBlocking && <Card className="mb-4 bg-amber-50 border border-amber-200 shadow-sm">
            <div className="p-3 flex items-center justify-between">
              <div className="flex items-center gap-2 text-amber-700">
                <Shield size={16} />
                <span className="text-sm font-medium">Verification required to continue</span>
              </div>
              <Button size="sm" variant="outline" className="text-xs bg-white" onClick={() => transcript.handleVerifySystemCheck('system')}>
                Verify Identity
              </Button>
            </div>
          </Card>}
        
        <ChatMessages messages={transcript.messages} isAgentMode={true} onSelectResponse={transcript.handleSelectResponse} onVerifySystemCheck={transcript.handleVerifySystemCheck} onValidateSensitiveData={transcript.handleValidateSensitiveData} messagesEndRef={transcript.messagesEndRef} onModuleComplete={handleInlineModuleComplete} />
      </ScrollArea>
      ...
      <div className="p-4 border-t">
        <div className="flex items-center justify-between pb-2 gap-3">
          <div className="flex-grow">
            <div className="flex items-center gap-2">
              {transcript.callActive ? <span className="flex items-center gap-1.5 text-green-500">
        ...
        <div className="flex justify-center pt-2">
          <span className="text-center text-xs text-gray-400">
            {transcript.awaitingUserResponse ? "Customer is waiting for your response" : "Waiting for customer input"}
          </span>
        </div>
      </div>

      {/* Module container that will display active modules */}
      {transcript.activeModule && <ModuleContainer moduleConfig={transcript.activeModule} onClose={transcript.closeModule} onComplete={handleModuleComplete} currentState={transcript.currentState} stateData={transcript.stateData} />}
    </div>;
};
